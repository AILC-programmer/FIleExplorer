...
namespace Explorer
{
    public partial class MainExplorer : MetroForm
    {
        ...
        private void MainExplorer_Load(object sender, EventArgs e)
        {
            ...
            var iconsFile = Path.Combine(Application.StartupPath, "Icons");
            if (!Directory.Exists(iconsFile)) Directory.CreateDirectory(iconsFile);
        ...
        {
            new UtilityForms.SettingsForm().ShowDialog();
        ...
        {
            var themeStyle = Properties.Settings.Default.appTheme;
        ...
        private void primarySetListView()
        {
            ...
            {
                ToolsVisible(false);
                var drives = Tools.Entities.GetDrives();

                ExplorerMListView.Items.Clear();
                PathToolStripLabel.Text = "PC";
                foreach (var drive in drives)
                {
                    if (drive.IsReady)
                        ExplorerMListView.Items.Add(new ListViewItem(drive.VolumeLabel + " (" + drive.Name + ")")
                        {
                            Tag = drive,
                            ToolTipText = Tools.Entities.GetDriveInfo(drive)
                        });
                    else
                        ExplorerMListView.Items.Add(new ListViewItem(drive.Name));
                }
            }
            catch (Exception ex)
            {
                ...
                foreach (var plugin in plugins)
                {
                    try
                    {
                        ...
                        foreach (var type in pluginTypes)
                        {
                            ...
                            var iconAttribute = type.GetCustomAttribute<PluginManager.IconAttribute>();
                            ...
                            if (iconAttribute != null)
                                toolStripMenuItem.Image = iconAttribute.image;

                            toolStripMenuItem.Tag = Activator.CreateInstance(type) as PluginManager.ICommand;
